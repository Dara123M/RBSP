...
def main():
  ...
  for k in KS:
    ...
    #oFiles = [f.replace(".txt", f"_{k:02}.txt") for f in oFiles]
    ...
    for i in range(NUM_INSTANCES):
      ...
      solution, cost = variableNeighborhoodSearch(instances[i][0], instances[i][1], NUM_NODES, ITER_MAX, k, None)
      end_time = datetime.datetime.now()
      times.append((end_time-start_time).total_seconds())
      solutions.append(solution)
      costs.append(cost)
      writeSolutionInFile(oFiles[i], times[i], solutions[i], costs[i],k)
